            ...
            success: function(response){
                if (!response) {
                    ...
                    for (let z = 0; z < (totalItems); z++) {
                        rows += "<tr><td class='fst_clm tbl_clm tbl_body'>"+response[0][z]['marcas_id']+"</td>";
                        rows += "<td class='scd_clm tbl_clm tbl_body'>"+response[0][z]['mar_titulo']+"</td>"; 
                        if(response[0][z]['mar_activo'] == 1){
                            rows += "<td class='fve_clm tbl_clm tbl_body'><div class='status sts_on switch_stts' data-single-id='"+response[0][z]['marcas_id']+"'>Activo</div></td>";
                        } else {
                            rows += "<td class='fve_clm tbl_clm tbl_body'><div class='status sts_off switch_stts' data-single-id='"+response[0][z]['marcas_id']+"'>Inactivo</div></td>";
                        }
                        rows += "<td class='six_clm tbl_clm tbl_body'>";
                        rows += "<a href='https://www.eximpar.com.py/admin/marcas-edit.php?q="+response[0][z]['mar_dbtitulo']+"' class='status sts_act'>Editar</a>";
                    ...
                    function tableLoad(){
                        $("#total_items").html("Mostrando <span>"+response[0].length+"</span> de <span>"+response[1]+"</span> marcas");
