...
class Records{
    ...
    public function list_consultas(){
        ...
        if ($_POST['orderby'] == "default") {
            ...
            $sort = "con_nombre DESC";
        }

        $sql = "SELECT * FROM consultas ORDER BY ".$sort." LIMIT ".$_POST['length'];
    ...
    public function status_change(){
        $sql = "SELECT * FROM consultas WHERE consultas_id = :consultas_id";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute(["consultas_id" => $_POST['consultas_id']]);
        $stmt = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if ($stmt[0]['con_estado'] === 'Pendiente') {
            $con_stts = 'Concluido';
        } else {
            $con_stts = 'Pendiente';
        }

        $sql = "UPDATE consultas SET con_estado = :estado WHERE consultas_id = :consultas_id";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([
