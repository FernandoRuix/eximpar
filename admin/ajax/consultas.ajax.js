            ...
            success: function(response){
                if (!response) {
                    ...
                    for (let z = 0; z < (totalItems); z++) {
                        rows += "<tr><td class='tbl_clm tbl_body'>"+response[0][z]['con_nombre']+" "+response[0][z]['con_apellido']+"</td>";
                        rows += "<td class='tbl_clm tbl_body'>"+response[0][z]['con_mail']+"</td>";
                        rows += "<td class='tbl_clm tbl_body'>"+response[0][z]['con_telefono']+"</td>";
                        rows += "<td class='tbl_clm tbl_body'>"+response[0][z]['con_detalles']+"</td>";
                        rows += "<td class='tbl_clm tbl_body'>"+response[0][z]['con_type']+"</td>";
                        rows += "<td class='tbl_clm tbl_body'>"+response[0][z]['con_fecha']+"</td>";
                        if(response[0][z]['con_estado'] == "Concluido"){
                            rows += "<td class='fve_clm tbl_clm tbl_body'><div class='status sts_on switch_stts' data-single-id='"+response[0][z]['consultas_id']+"'>Concluido</div></td>";
                        } else {
                            rows += "<td class='fve_clm tbl_clm tbl_body'><div class='status sts_mdm switch_stts' data-single-id='"+response[0][z]['consultas_id']+"'>Pendiente</div></td>";
                        }
                    }
